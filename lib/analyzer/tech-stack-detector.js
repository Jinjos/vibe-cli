    ...
    this.frontendDetectors = {
      ...
      nextjs: {
        ...
        confidence: (evidence) => {
          ...
          if (evidence.scripts.some(s => s.includes('next'))) return 0.95;
    ...
    this.backendDetectors = {
      ...
      fastify: {
        ...
        confidence: (evidence) => {
          ...
          return evidence.codePatterns.length > 0 ? 0.7 : 0;
        }
      },
    ...
    for (const [db, config] of Object.entries(this.databaseDetectors)) {
      ...
      if (packageData && config.dependencies) {
        ...
        if (process.env.DEBUG_VIBE) {
          console.log(`\nDetecting ${db}:`);
          console.log('  Looking for:', config.dependencies);
          console.log('  Found in package.json:', evidence.dependencies);
          console.log('  All deps sample:', allDeps.slice(0, 5));
        }
      }
