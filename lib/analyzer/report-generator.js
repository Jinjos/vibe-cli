...
class ReportGenerator {
  ...
  generateOverview(techStack, projectPath) {
    ...
    if (techStack.languages && techStack.languages.length > 0) {
      const primaryLang = techStack.primary || techStack.languages[0] || 'Unknown';
      lines.push(`Primary Language: ${chalk.yellow(this.capitalize(primaryLang))}`);
      if (techStack.languages.length > 1) {
        const otherLanguages = techStack.languages.filter(l => l !== primaryLang);
        if (otherLanguages.length > 0) {
          lines.push(`Other Languages: ${otherLanguages.join(', ')}`);
        }
      }
    }
    ...
    if (techStack.frameworks && techStack.frameworks.length > 0) {
      const frontend = techStack.frameworks.filter(f => 
        ['react', 'vue', 'angular', 'svelte', 'nextjs', 'gatsby', 'nuxt'].includes(f.toLowerCase())
      );
      const backend = techStack.frameworks.filter(f => 
        ['express', 'fastapi', 'fastify', 'nestjs', 'django', 'flask', 'rails', 'laravel'].includes(f.toLowerCase())
      );
    ...
    if (techStack.buildTools && techStack.buildTools.length > 0) {
    ...
    if (techStack.architecture && techStack.architecture.patterns && techStack.architecture.patterns.length > 0) {
      lines.push('');
  ...
  /**
   * Identify missing tech-specific coverage with detailed requirements
   */
  identifyMissingTechCoverage(techStack, dedicatedRuleCategories) {
    const missing = [];
    
    // Technology-specific requirements mapping
    const techRequirements = {
      mongodb: {
        name: 'MongoDB',
        requirements: [
    ...
    for (const db of techStack.databases) {
      if (!dedicatedRuleCategories.has('Database')) {
        const dbConfig = techRequirements[db.toLowerCase()];
        if (dbConfig) {
          missing.push({
            tech: dbConfig.name,
            requirements: dbConfig.requirements,
            category: 'Database'
          });
        }
      }
    ...
    for (const test of techStack.testing) {
      if (!dedicatedRuleCategories.has('Testing')) {
        const testConfig = techRequirements[test.toLowerCase()];
        if (testConfig) {
          missing.push({
            tech: testConfig.name,
    ...
    for (const framework of techStack.frameworks) {
      const frameworkConfig = techRequirements[framework.toLowerCase()];
      if (frameworkConfig && !dedicatedRuleCategories.has('API Design')) {
        missing.push({
          tech: frameworkConfig.name,
  ...
  generateGapsSection(gaps, techStack) {
    ...
    if (gaps.priority.high.length > 0 || gaps.priority.medium.length > 0) {
      ...
      for (const [tech, needs] of Object.entries(techNeeds)) {
        ...
        // Show individual requirements if available, otherwise show the general detail
        const firstNeed = needs[0];
        
        if (firstNeed.requirements && firstNeed.requirements.length > 0) {
          // Use the detailed requirements array
          for (const requirement of firstNeed.requirements) {
            lines.push(`  ${firstNeed.priority === 'high' ? chalk.red('•') : chalk.yellow('•')} ${requirement}`);
          }
        } else {
          // Fallback: split comma-separated details into individual bullet points
          for (const need of needs) {
            // If detail contains commas, split them into separate bullets
            if (need.detail && need.detail.includes(',')) {
              const details = need.detail.split(',').map(d => d.trim());
              for (const detail of details) {
                if (detail) {
                  lines.push(`  ${need.priority === 'high' ? chalk.red('•') : chalk.yellow('•')} ${detail}`);
                }
              }
            } else {
              // Single detail
              const detail = need.detail || need.title || 'Unknown requirement';
              lines.push(`  ${need.priority === 'high' ? chalk.red('•') : chalk.yellow('•')} ${detail}`);
            }
          }
        }
  ...
  groupGapsByTechnology(gaps, techStack) {
    ...
    for (const gap of allGaps) {
      ...
      } else if (gap.category) {
        // Handle core categories like 'Git Workflow'
        if (gap.category === 'Git Workflow') {
          tech = 'General';
        } else {
          tech = gap.category;
        }
      ...
      grouped[tech].push({
        ...
        requirements: gap.requirements || [] // Include requirements array if available
      });
    }
