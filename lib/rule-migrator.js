...
async function migrateExistingRules(projectPath, vibeDir, existingRules) {
  ...
  console.log(`   📋 Processing ${rulesToMigrate.length} existing rule files...\n`);
  ...
  for (const rule of rulesToMigrate) {
    try {
      const migrated = await migrateRuleFile(rule, vibeDir, nameAnalysis);
      if (migrated) {
        migratedRules.push({
          ...migrated,
        ...
        console.log(`      ✅ ${chalk.green(rule.relativePath)} → ${chalk.cyan(migrated.filename)}`);
      }
    } catch (error) {
 ...
 */
async function migrateRuleFile(rule, vibeDir, nameAnalysis) {
  ...
  try {
    ...
    console.log(`      ⚠️  YAML parse error in ${rule.relativePath}, attempting manual extraction`);
