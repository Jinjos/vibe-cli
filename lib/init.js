...
async function initializeVibeSystem(projectPath, options = {}) {
  ...
  try {
    ...
    if (!options.full && migratedRules.length > 0) {
      showCleanupInformation(migratedRules, log);
    }

    // Step 7: Show completion summary
    displayCompletionSummary(detection, generatedConfigs, options.full, platformsToSetup, log);
 ...
 */
function showCleanupInformation(migratedRules, log) {
  ...
  if (cursorRules > 0 || copilotRules > 0) {
    ...
    log('   Your rules have been migrated to the vibe/ directory, but the original files remain:');
    
    if (cursorRules > 0) {
      log(`   📁 .cursor/rules/ - ${cursorRules} original Cursor files`);
    }
    if (copilotRules > 0) {
      log(`   📁 .github/ - ${copilotRules} original Copilot files`);
    }
    
    log('\n   After testing the vibe system, you can safely remove the originals:');
    log(`   ${chalk.cyan('vibe cleanup')} - Remove original files only`);
    log(`   ${chalk.cyan('vibe init --full')} - Next time, use this for end-to-end setup`);
 ...
 */
function displayCompletionSummary(detection, generatedConfigs, isFullSetup, selectedPlatforms, log) {
  log(chalk.green('\n🎉 Vibe system initialization complete!\n'));
  
  log(chalk.bold('📋 What was created:'));
  log(`   📁 ${chalk.cyan('vibe/')} - Your unified rules directory`);
  
  if (generatedConfigs.length > 0) {
    log('   🤖 Platform configurations:');
    generatedConfigs.forEach(config => {
      log(`      → ${chalk.yellow(config)}`);
    });
  }

  // Show which platforms were configured
  if (selectedPlatforms.length < 4) {
    log(`   🎯 Configured platforms: ${selectedPlatforms.join(', ')}`);
    const allPlatforms = ['cursor', 'claude', 'copilot', 'gemini'];
    const skipped = allPlatforms.filter(p => !selectedPlatforms.includes(p));
    if (skipped.length > 0) {
      log(`   ⚪ Skipped platforms: ${skipped.join(', ')}`);
    }
  }

  if (isFullSetup) {
    log('   🧹 Original rule files removed');
  }

  log(chalk.bold('\n💡 Next steps:'));
  log('   1. Review and customize rules in vibe/ directory');
  log('   2. Test with your AI tools to verify integration');
  log('   3. Run', chalk.cyan('vibe status'), 'to verify setup');
  
  if (!isFullSetup && detection.existingRules.length > 0) {
    log('   4. Run', chalk.cyan('vibe cleanup'), 'to remove original rule files');
  }
