...
async function cleanupOriginalRules(projectPath, migratedRules, options = {}) {
  ...
  try {
    ...
    if (migratedRules.length === 0) {
      logger(chalk.blue('ℹ️  No original rules to cleanup'));
      logger(chalk.blue('💡 Run'), chalk.cyan('vibe init'), chalk.blue('first to migrate rules\n'));
      return;
    }
    ...
    if (cleanupPlan.length === 0) {
      logger(chalk.blue('ℹ️  No original rule files need cleanup'));
      logger(chalk.gray('   Root CLAUDE.md and GEMINI.md were enhanced, not replaced'));
      logger(chalk.gray('   Only directory-based rules are removed\n'));
      return;
    }

    // Show what will be cleaned up
    logger(chalk.bold('🗂️  Original files to be removed:\n'));
    cleanupPlan.forEach(item => {
      logger(`   ${chalk.red('🗑️')} ${item.relativePath} ${chalk.gray(`(migrated to vibe/${item.newName})`)}`);
    });

    if (options.dryRun) {
      logger(chalk.yellow('\n🔍 Dry run mode - no files removed'));
      logger(chalk.yellow('💡 Run without --dry-run to perform cleanup\n'));
      return cleanupPlan;
    }

    // Always create backups (no option to disable)
    spinner.start('📦 Creating backups...');
    await createCleanupBackups(cleanupPlan, logger);
    ...
    // Show results
    showCleanupResults(results, logger);
 ...
 */
async function createCleanupBackups(cleanupPlan, logger) {
  for (const item of cleanupPlan) {
    try {
      ...
      logger.warn(`⚠️  Could not backup ${item.relativePath}: ${error.message}`);
    }
  }
}
 ...
 */
function showCleanupResults(results, logger) {
  logger(chalk.bold('\n🎉 Cleanup Results:\n'));
  ...
  if (results.removed > 0) {
    ...
    logger(chalk.blue('📁 All rules preserved in vibe/ directory'));
    logger(chalk.cyan('🤖 Vibe system configurations remain intact'));
  }
  
  if (results.protected > 0) {
    logger(chalk.blue(`🛡️  ${results.protected} files protected (vibe system or failed)`));
  }
