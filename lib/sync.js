...
async function syncVibeSystem(projectPath, options = {}) {
  ...
  try {
    ...
    if (rulesToSync.length === 0) {
      ...
      logger(chalk.green('✨ No platform rules to sync!'));
    ...
    if (options.dryRun) {
      ...
      logger(`   ${chalk.green('✨')} ${syncAnalysis.new.length} new rules to add`);
      logger(`   ${chalk.yellow('🔄')} ${syncAnalysis.overwrite.length} existing rules to overwrite`);
      
      if (syncAnalysis.overwrite.length > 0) {
        logger(chalk.yellow('\n📝 Rules that will be overwritten:'));
        syncAnalysis.overwrite.forEach(rule => {
          logger(`   ${chalk.yellow('→')} ${rule.targetName}`);
        });
      }
    ...
    logger(chalk.green(`\n✨ Sync complete!`));
    logger(`   ${chalk.green('✨')} ${syncAnalysis.new.length} new rules added`);
    logger(`   ${chalk.yellow('🔄')} ${syncAnalysis.overwrite.length} existing rules overwritten`);
    logger(`   ${chalk.blue('📁')} Total rules synced: ${syncedRules.length}`);
