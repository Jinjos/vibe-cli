...
async function showStatus(projectPath, options = {}) {
  console.log(chalk.magenta('🔥 Vibe System Status\n'));
  ...
  if (!vibeExists) {
    ...
    console.log(chalk.yellow('💡 Run'), chalk.cyan('vibe init'), chalk.yellow('to get started'));
    return;
  }

  console.log(chalk.green('✅ Vibe system initialized\n'));

  // Detect platforms and analyze configurations
  const detection = await detectPlatforms(projectPath);
  await showPlatformStatus(projectPath, detection.platforms, options.verbose);

  // Analyze vibe rules
  const vibeRules = await analyzeVibeRules(vibeDir);
  await showRuleStatus(vibeRules, detection.platforms, options.verbose);

  // Performance analysis per platform
  if (detection.platforms.length > 0) {
    await showPerformanceAnalysis(vibeRules, detection.platforms, options.verbose);
  }

  // Error detection and validation
  const issues = await detectRuleIssues(vibeRules);
  await showIssues(issues);

  // Show suggestions
  showNextSteps(issues.length > 0, options.verbose);
}

/**
 * Show platform detection and configuration status
 */
async function showPlatformStatus(projectPath, detectedPlatforms, verbose) {
  ...
  for (const platform of ['cursor', 'copilot', 'claude', 'gemini']) {
    ...
    if (isDetected && configExists) {
      console.log(`   ✅ ${chalk.green(limits.name.toUpperCase())} (${limits.configFile})`);
    } else if (isDetected && !configExists) {
      console.log(`   ⚠️  ${chalk.yellow(limits.name.toUpperCase())} (detected but no vibe config)`);
    } else {
      console.log(`   ⚪ ${chalk.gray(limits.name.toUpperCase())} (not configured)`);
    }

    // VERBOSE: Show additional platform details
    if (verbose && (isDetected || limits)) {
      console.log(`      📊 Context limit: ${limits.contextWindow.toLocaleString()} tokens`);
      console.log(`      ⚠️  Warning threshold: ${limits.warningThreshold.toLocaleString()} tokens`);
      console.log(`      📝 ${limits.notes}`);
      console.log('');
    }
  }
 ...
 */
async function showRuleStatus(vibeRules, detectedPlatforms, verbose) {
  ...
  if (verbose) {
    ...
    console.log(chalk.bold(`📁 Vibe Rules by Enforcement (${vibeRules.length} files):\n`));
    showRuleCategory('⚡ Always Apply', categories.always, chalk.red);
    showRuleCategory('🎯 File-Specific', categories.fileSpecific, chalk.yellow);
    showRuleCategory('🧠 Intelligent', categories.intelligent, chalk.blue);
    showRuleCategory('🔧 Manual Only', categories.manual, chalk.gray);

    console.log(chalk.bold('📊 Detailed Rule Analysis:\n'));
    for (const rule of vibeRules) {
      const tokens = estimateTokens(rule.content);
      console.log(`   📄 ${chalk.bold(rule.filename)}`);
      console.log(`      🔤 ~${tokens.toLocaleString()} tokens, ${rule.content.length.toLocaleString()} characters`);
      console.log(`      ⚙️  ${getEnforcementDescription(rule.frontmatter)}`);
      console.log(`      📝 "${rule.frontmatter.description || 'No description'}"`);
      console.log(''); // Add spacing between rules
    }
  } else {
    // REGULAR: Show simple summary only
    console.log(chalk.bold(`📁 Vibe Rules (${vibeRules.length} files): `), 
 ...
 */
async function showPerformanceAnalysis(vibeRules, detectedPlatforms, verbose) {
  console.log(chalk.bold('📊 Platform Performance Analysis:\n'));
  ...
  for (const platform of detectedPlatforms) {
    ...
    console.log(`${statusIcon} ${statusColor(limits.name.toUpperCase())} (${limits.contextWindow.toLocaleString()} context limit):`);
    console.log(`   📊 Current rules: ${analysis.totalTokens.toLocaleString()} tokens (${analysis.percentage}% of context)`);
    
    if (warningLevel === 'high') {
      console.log(`   ${chalk.red('🚨 HIGH IMPACT')} - May significantly slow responses`);
    } else if (warningLevel === 'moderate') {
      console.log(`   ${chalk.yellow('💛 MODERATE IMPACT')} - Monitor performance`);
    } else {
      console.log(`   ${chalk.green('✅ LOW IMPACT')} - Performance should be good`);
    }
    ...
    if (verbose) {
      ...
      console.log(`   📈 Always Apply rules: ${alwaysApplyTokens.toLocaleString()} tokens`);
      console.log(`   📈 File-Specific rules: ${(analysis.totalTokens - alwaysApplyTokens).toLocaleString()} tokens`);
      console.log(`   📈 Available context: ${(limits.contextWindow - analysis.totalTokens).toLocaleString()} tokens`);
      console.log(`   📝 Platform notes: ${limits.notes}`);
    }
...
async function showIssues(issues) {
  if (issues.length === 0) {
    console.log(chalk.green('✅ No rule configuration issues detected\n'));
    return;
  }

  console.log(chalk.bold(`⚠️  Found ${issues.length} issues with vibe rules:\n`));

  for (const issue of issues) {
    const severity = issue.severity === 'error' ? chalk.red('❌') : chalk.yellow('⚠️');
    console.log(`${severity} ${chalk.bold(issue.filename)}:`);
  ...
  console.log(chalk.yellow('💡 Run'), chalk.cyan('vibe fix'), chalk.yellow('to automatically resolve these issues\n'));
}

/**
 * Show next steps to user
 */
function showNextSteps(hasIssues, verbose) {
  console.log(chalk.bold('💡 Next Steps:'));
  
  if (hasIssues) {
    console.log(`   1. Run ${chalk.cyan('vibe fix')} to resolve configuration issues`);
    console.log(`   2. Test with your AI tools to verify performance`);
  } else {
    console.log(`   1. Test with your AI tools to verify integration`);
    console.log(`   2. Add custom rules by editing files in ${chalk.cyan('vibe/')} directory`);
  }
  
  if (!verbose) {
    console.log(`   3. Run ${chalk.cyan('vibe status --verbose')} for detailed analysis`);
  }
  console.log('');
}
 ...
 * Helper functions
 */
function showRuleCategory(title, rules, colorFn) {
  console.log(`${colorFn(title)} (${rules.length}):`);
  if (rules.length === 0) {
    console.log('   (none)');
  } else {
    rules.forEach(rule => {
      const description = getEnforcementDescription(rule.frontmatter);
      console.log(`   ${rule.filename} ${chalk.gray(`(${description})`)}`);
    });
  }
  console.log(''); // Add spacing after each category
}
