...
async function fixRuleIssues(projectPath, options = {}) {
  ...
  try {
    logger(chalk.magenta('🔧 Vibe Fix - Resolving Rule Issues\n')); // ✅ Use logger
    ...
    try {
      ...
      logger(chalk.yellow('💡 Run'), chalk.cyan('vibe init'), chalk.yellow('first'));
      return;
    }
    ...
    if (issues.length === 0) {
      logger(chalk.green('✅ No issues detected - your vibe is clean!\n'));
      return;
    }

    // Show what will be fixed
    logger(chalk.bold('🔧 Issues to fix:\n'));
    issues.forEach(issue => {
      const icon = issue.severity === 'error' ? chalk.red('❌') : chalk.yellow('⚠️');
      logger(`${icon} ${chalk.bold(issue.filename)}: ${issue.message}`);
    });

    if (options.dryRun) {
      logger(chalk.yellow('\n🔍 Dry run mode - no changes made'));
      logger(chalk.yellow('💡 Run without --dry-run to apply fixes'));
      return;
    }
    ...
    if (options.backup) {
      ...
      await createBackups(vibeDir, issues, logger);
    ...
    // Show results
    showFixResults(fixResults, logger);
 ...
 */
async function createBackups(vibeDir, issues, logger) {
  ...
  for (const filename of filesToBackup) {
    ...
    try {
      ...
      logger.warn(`⚠️  Could not backup ${filename}: ${error.message}`);
    }
  }
}
 ...
 */
function showFixResults(results, logger) {
  logger(chalk.bold('\n🎉 Fix Results:\n'));
  ...
  if (results.skipped > 0) {
    logger(chalk.yellow(`⚠️  ${results.skipped} issues require manual attention`));
  }
